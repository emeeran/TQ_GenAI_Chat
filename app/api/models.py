...
def get_models_by_provider(provider):
    ...
    try:
        ...
        if not provider_instance:
            return jsonify({"error": f"Provider {provider} not available", "models": []}), 404

        # Get models from the provider
        models = provider_instance.get_models()

        return jsonify(
            {"provider": provider, "models": models, "default": models[0] if models else None}
        )

    except Exception as e:
        return (
            jsonify({"error": f"Failed to get models for {provider}: {str(e)}", "models": []}),
            500,
        )
