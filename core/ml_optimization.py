    ...
    async def recommend_provider(self, user_id: str, message: str, 
        ...
        try:
            ...
            if hasattr(self.provider_predictor, 'predict_proba'):
                ...
                for provider, prob in zip(classes, provider_probs):
    ...
    async def _retrain_models(self):
        ...
        try:
            ...
            if sum(satisfaction_mask) > 10:
                X_sat = [x for x, mask in zip(X, satisfaction_mask) if mask]
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        result = await optimizer.process_chat_request(
            user_id, 
            "What is machine learning?", 
            available_providers
        )
        
        print("Chat request result:", json.dumps(result, indent=2, default=str))
        ...
        predictions = await optimizer.get_usage_predictions(24)
        print("Usage predictions:", json.dumps(predictions, indent=2, default=str))
        
        # Get metrics
        metrics = await optimizer.get_optimization_metrics()
        print("Optimization metrics:", json.dumps(metrics, indent=2, default=str))
        
        print("ML-powered optimization example completed!")
